...
# Pre-defined configs
##
from isaaclab_assets.robots.nova import NOVA_CFG  # isort: skip


@configclass
class NovaReachEnvCfg(joint_pos_env_cfg.NovaReachEnvCfg):
    def __post_init__(self):
        # post init of parent
        super().__post_init__()

        # Set Nova as robot
        # We remove stiffness and damping for the shoulder and wrist joints for effort control
        self.scene.robot = NOVA_CFG.replace(prim_path="{ENV_REGEX_NS}/Robot")
        self.scene.robot.actuators["nova_shoulder"].stiffness = 0.0
        self.scene.robot.actuators["nova_shoulder"].damping = 0.0
        self.scene.robot.actuators["nova_wrist"].stiffness = 0.0
        self.scene.robot.actuators["nova_wrist"].damping = 0.0
